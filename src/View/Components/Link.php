...
class Link extends Component
{
    ...
    {
        return <<<'HTML'
            <a {{ $attributes->merge(['class' => $colorTheme() . ' inline-flex items-center justify-center gap-2 disabled:opacity-75 delay-50 duration-300 ease-in-out rounded-md px-4 py-2 tracking-widest text-base focus:outline-none focus:ring-0']) }}>
            ...
            <!-- ICON -->
            @unless(empty($icon))
            <span class="flex items-center" @if($spinner) wire:loading.class="hidden" wire:target="{{ $spinnerTarget() }}" @endif>
